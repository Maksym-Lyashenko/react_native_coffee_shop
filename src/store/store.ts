...
export const useStore = create<IStore>()(
    persist(
        (set, get) => ({
            ...
            CartList: [],
            OrderHistoryList: [],
            addToCart: (cartItem: TCartItem) => set(produce(state => {
                let found = false;
                for (let i = 0; i < state.CartList.length; i++) {
                    if (state.CartList[i].id == cartItem.id) {
                        ...
                        state.CartList[i].prices.sort((a: { size: string }, b: { size: string }) => {
            ...
            calculateCartPrice: () => set(produce(state => {
                ...
                for (let i = 0; i < state.CartList.length; i ++) {
                    ...
                    ttlPrice = ttlPrice + tempPrice;
                }
                state.CartPrice = ttlPrice.toFixed(2).toString();
            })),
